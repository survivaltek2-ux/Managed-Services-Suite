...
async function upsertReplitUser(claims: Record<string, unknown>) {
  ...
  const existing = await db
    .select()
    .from(usersTable)
    .where(eq(usersTable.replitUserId, replitSub))
    .limit(1);
  ...
  const name = [firstName, lastName].filter(Boolean).join(" ") || email || replitSub;
  const [user] = await db
    .insert(usersTable)
    .values({
      name,
      email: email || `${replitSub}@replit.local`,
      password: "",
      company: "Unknown",
      replitUserId: replitSub,
    })
    .returning();
...
router.get("/callback", async (req: Request, res: Response) => {
  ...
  const sessionData: SessionData = {
    user: {
      id: dbUser.replitUserId || String(dbUser.id),
      email: dbUser.email,
      firstName: null,
      lastName: null,
